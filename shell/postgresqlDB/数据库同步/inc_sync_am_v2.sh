...
psql --command "insert into ems_pm_files(source_id, file_name, update_time) select source_id, file_name, update_time from tmp_inc_ems_pm_files on conflict(source_id,file_name) do update set file_name=excluded.file_name, update_time=excluded.update_time;" "$DestConn"
